...
@timeit
def generate_point_cloud(feature_matches_list, K_matrix, Sdescriptors):
    """
    ...
    """
    point_cloud = []
    for img1, img2, matches in feature_matches_list:
        # extract keypoints from matches
        pts1 = np.float32([Sdescriptors[img1][match.queryIdx].pt for match in matches])
        pts2 = np.float32([Sdescriptors[img2][match.trainIdx].pt for match in matches])
        # pts1 = np.float32([kp.pt for kp in [matches[i].queryIdx for i in range(len(matches))]])
        # pts2 = np.float32([kp.pt for kp in [matches[i].trainIdx for i in range(len(matches))]])
        E, _ = OpenCV.findEssentialMat(pts1, pts2, K_matrix)
        R1, R2, t = OpenCV.decomposeEssentialMat(E)

        # Triangulate the points to get 3D coordinates
        pts3d = OpenCV.triangulatePoints(K_matrix.dot(np.hstack((np.eye(3), np.zeros((3, 1))))), 
                                      K_matrix.dot(np.hstack((R1[0], t))), pts1.T, pts2.T)
        pts3d = pts3d.T[:, :3] / pts3d.T[:, 3:]

        # Estimate extrinsic parameters for each image using OpenCV.solvePnP()
        _, rvec1, tvec1 = OpenCV.solvePnP(objectPoints=pts3d, imagePoints=pts1, cameraMatrix=K_matrix, distCoeffs=None)
        _, rvec2, tvec2 = OpenCV.solvePnP(objectPoints=pts3d, imagePoints=pts2, cameraMatrix=K_matrix, distCoeffs=None)

        # Convert rotation vectors to rotation matrices
        R1, _ = OpenCV.Rodrigues(rvec1)
        R2, _ = OpenCV.Rodrigues(rvec2)

        for i in range(len(R1)):
            P1 = np.hstack((R1[i], tvec1[i]))
            P2 = np.hstack((R2[i], tvec2[i]))
            pts_3d = OpenCV.triangulatePoints(K_matrix.dot(P1), K_matrix.dot(P2), pts1.T, pts2.T)
            pts_3d = pts_3d.T[:, :3] / pts_3d.T[:, 3:]
...
def davinci_run(image_set_name):
    ...
    points_cloud = generate_point_cloud(feature_matches, K_matrix, descriptors)
